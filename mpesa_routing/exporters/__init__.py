...
class ReportGenerator:
    ...
    def generate_text(self, scenario: Scenario, results: Dict[str, SimulationResult]) -> str:
    ...
    def _row(self, label: str, naive_val, opt_val) -> str:
        fmt = ",.1f" if isinstance(naive_val, float) else ""
        n = f"{naive_val:>15.1f}" if isinstance(naive_val, float) else f"{naive_val:>15}"
        o = f"{opt_val:>15.1f}" if isinstance(opt_val, float) else f"{opt_val:>15}"
        return f"{label:25} {n} {o}"

    def generate_all(self, comparisons: list) -> str:
        parts = [self.generate_text(s, r) for s, r in comparisons]
        return "\n\n".join(parts)
